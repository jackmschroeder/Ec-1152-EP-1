...
```{r summit ranks, echo=FALSE, include=FALSE}
# The following code allowed me to look at Summit's tracts in relation to the rest of
# Union County. Summit's tracts are 37700, 37800, 37900, and 38000.

summit_ranks <- county %>% 
  select(tract, kfr_pooled_p25) %>% 
  arrange(desc(kfr_pooled_p25))
```

In many outcomes, Summit is ahead of the rest of Union County. Of the 108 Census tracts within the county, Summit's tracts rank 2nd, 31st, 45th, and 53rd in the pooled household incomes of low-income children (`kfr_pooled_p25`). When displayed cartographically, Summit's outcomes for low-income children appear better than those of many other towns in the county (which are located to the southeast of Summit).
...
The Opportunity Atlas data looks at children born between 1978 and 1983 and provides insights on their outcomes through adulthood. Using this data to look at Summit's outcomes may be misleading, since Summit itself has changed since the 1980s. After a population boom in the 1950s, the population began to precipitously decline in the 1970s. This decline continued until the late 1990s, but the city's population has yet to reach its high of 23,677 in 1960 (Census Data). Many in Summit believe that the city's population recovery was accompanied by fundamental changes in the town's identity, completing a transition into an upper-class Wall Street exclave in Union County. <ARE THESE CHANGES IMPORTANT> <WHAT DATA TO TEST WHETHER NEIGHBORHOOD HAS CHANGED>
...
q3.3 <- mean(county$kfr_pooled_p25)
q3.3 <- round(q3.3, digits=0)
q3.4 <- mean(tract$kfr_pooled_p25)
...
q5.3 <- mean(county$kfr_pooled_p75)
q5.3 <- round(q5.3, digits=0)
q5.4 <- mean(tract$kfr_pooled_p75)
q5.4 <- round(q5.4, digits=0)

q5.5 <- sd(oppatlas$kfr_pooled_p75)
q5.5 <- round(q5.5, digits=0)
q5.6 <- sd(state$kfr_pooled_p75)
q5.6 <- round(q5.6, digits=0)
q5.7 <- sd(county$kfr_pooled_p75)
q5.7 <- round(q5.7, digits=0)
q5.8 <- sd(town$kfr_pooled_p75)
q5.8 <- round(q5.8, digits=0)

q5.9 <- mean(oppatlas$kfr_pooled_p100)
q5.9 <- round(q5.9, digits=0)
q5.10 <- mean(state$kfr_pooled_p100)
q5.10 <- round(q5.10, digits=0)
q5.11 <- mean(county$kfr_pooled_p100)
q5.11 <- round(q5.11, digits=0)
q5.12 <- mean(tract$kfr_pooled_p100)
q5.12 <- round(q5.12, digits=0)

q5.13 <- sd(oppatlas$kfr_pooled_p100)
q5.13 <- round(q5.13, digits=0)
q5.14 <- sd(state$kfr_pooled_p100)
q5.14 <- round(q5.14, digits=0)
q5.15 <- sd(county$kfr_pooled_p100)
q5.15 <- round(q5.15, digits=0)
q5.16 <- sd(town$kfr_pooled_p100)
